...
class ConvBlock(nn.Module):
    def __init__(self, in_channel, mid_channel, out_channel, last_relu=True):
        ...
        self.conv1 = nn.Conv1d(in_channel, mid_channel, 3, 1, 1)
        self.bn1 = nn.BatchNorm1d(mid_channel)
        self.conv2 = nn.Conv1d(mid_channel, out_channel, 3, 1, 1)
...
class DownSample(nn.Module):
    def __init__(self, channel):
        ...
        self.conv = nn.Conv1d(channel, channel, 3, 2, 1)
...
class UpSample(nn.Module):
    def __init__(self, channel):
        ...
        self.conv = nn.ConvTranspose1d(channel, channel, 3, 2, 1, output_padding=1)
        self.bn = nn.BatchNorm1d(channel)
        self.relu = nn.LeakyReLU(inplace=True)
...
class AutoEncoder(nn.Module):
    ...
    def slope_loss(self, pred, true):
        ...
        pred_slope = torch.abs(pred_shifted_left - pred_shifted_right)

        true_shift_right = true[:, :, 1:]
        true_shift_left = true[:, :, :-1]
        true_slope = torch.abs(true_shift_left - true_shift_right)
...
class Res_Encoder(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=[3, 4, 6, 3], in_channel=21, strategy='mean', zero_init_residual=False):
...
# ---------------------------------------Resnet_Mem_Encoder-----------------------------------
class Res_Mem_Encoder(nn.Module):
    def __init__(self, block=BasicBlock, num_blocks=[3, 4, 6, 3], in_channel=21, strategy='mean', zero_init_residual=False):
...
class Res_Mem_Decoder(nn.Module):
    def __init__(self, with_last_relu=False):
...
class Resnet_MemEncoder(nn.Module):
    ...
    def EntropyLoss(self, similarity, eps=1e-12):
        ...
        res = similarity * torch.log(similarity + eps)  # [BT, N]
...
class SkipBlock(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        self.conv1 = nn.Conv1d(in_channel, out_channel, 3, 1, 1)
        self.bn1 = nn.BatchNorm1d(out_channel)
        self.upsample = UpSample(out_channel)
        self.conv2 = nn.Conv1d(in_channel, out_channel, 3, 1, 1)
        self.bn2 = nn.BatchNorm1d(out_channel)
        self.relu = nn.LeakyReLU(inplace=True)
...
class MemModule(nn.Module):
    ...
    def forward(self, X):
        ...
        similarity = F.linear(X, self.proto)  # [BT, 512] * [512, N] = [BT, N]
        similarity = F.softmax(similarity, dim=1)  # [BT, N]
        if self.shrink_thres > 0:
            similarity = self.hard_shrink_relu(similarity, lambd=self.shrink_thres)
            similarity = F.normalize(similarity, p=1, dim=1)  # [BT, N]
        mem_trans = self.proto.permute(1, 0)  # [512, N]
        proto_based_fea = F.linear(similarity, mem_trans)  # [BT, N] *  [N, 512] = [BT, 512]
...
class VAEEncoder(nn.Module):
    def __init__(self, dim=1000):
        ...
        self.conv1 = nn.Conv1d(10, 8, 4, 2, 1)
        self.bn1 = nn.BatchNorm1d(8)
        self.conv2 = nn.Conv1d(8, 16, 4, 2, 1)
        self.bn2 = nn.BatchNorm1d(16)
        self.act = nn.LeakyReLU()
        self.fc = nn.Linear(16 * dim // 4, 1000)
        self.fc_mean = nn.Linear(1000, 10)
        self.fc_var = nn.Linear(1000, 10)
...
class VAEDecoder(nn.Module):
    def __init__(self, dim):
        ...
        self.fc2 = nn.Linear(1000, 16 * dim // 4)
        self.conv1 = nn.ConvTranspose1d(16, 8, 4, 2, 1)
        self.bn1 = nn.BatchNorm1d(8)
        self.conv2 = nn.ConvTranspose1d(8, 10, 4, 2, 1)
        self.act = nn.LeakyReLU()
...
class VAE(nn.Module):
    def __init__(self, dim=1000, num_z=5, reductions='mean'):
        ...
        self.Loss = torch.nn.L1Loss(reduction=reductions)
    ...
    def loss(self, X, turelabel):
        ...
        kld_Loss = 0.8 * torch.mean(-0.5 * torch.sum(1 + var - mean ** 2 - var.exp(), dim=1), dim=0)
...
class EstimatorAutoEncoder(nn.Module):
    ...
    def Autoregressionloss(self, z, z_dist, eps=finfo(float).eps):
        ...
        index = torch.clamp(torch.unsqueeze(z_d, dim=1) * self.cpd_dim, min=0,
...
class EstimatorMemAutoEncoder(nn.Module):
